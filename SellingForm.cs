...
namespace Shop
{
    public partial class SellingForm : Form
    {
        ...
        private void panel1_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                Con.Open();
                string query = "SELECT AttName FROM AttTable";
                SqlCommand cmd = new SqlCommand(query, Con);
                SqlDataReader reader = cmd.ExecuteReader();

                if (reader.Read())
                {
                    string sellerName = reader["AttName"].ToString();
                    sellername.Text = sellerName;
                }
                else
                {
                    sellername.Text = "Seller Name Not Found";
                }
        ...
        {
            Con.Open();
            string query = "select ProdName, Quantity, Price from ProdTable1";
        ...
        {
            Con.Open();
            string query = "select ProdName, Quantity, Price from ProdTable1 where category='" + categoryS.SelectedValue.ToString() + "'";
            SqlDataAdapter sda = new SqlDataAdapter(query, Con);
            SqlCommandBuilder builder = new SqlCommandBuilder(sda);
            var data = new DataSet();
            sda.Fill(data);
        ...
        {
            Con.Open();
            string query = "select ProdName, Quantity, Price from ProdTable1 where ProdName like '" + "%" + search.Text + "%" + "'";
            SqlDataAdapter sda = new SqlDataAdapter(query, Con);
            SqlCommandBuilder builder = new SqlCommandBuilder(sda);
            var data = new DataSet();
            sda.Fill(data);
        ...
        private void calculate_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    change.Text = "Rs. " + Convert.ToString(Convert.ToInt32(paid.Text) - Convert.ToInt32(Gtotal));
                }
            }
        }
        ...
        {
            e.Graphics.DrawString("Nabeel Shop", new Font("Ariel", 30, FontStyle.Bold), Brushes.Maroon, new Point(250));
            ...
            string top2 = "********************************************************\n\n";
            string topunder = "--------------------------------------------------------\n\n";
            string top3 = "Item\tPrice(Rs.)\tQty\tTotal(Rs.)\n";
            string down = "***********************************************************\n";
            ...
            while (n < activesale.Rows.Count)
            {
                ...
                UpdateProdQty("update ProdTable1 set Quantity=(Quantity -" + Convert.ToInt32(QtyU) + ") where ProdName='" + NameU + "'" + " and Price=" + PriceU + ";");

                n++;
                pt += 50;
            }
            String Paid = "Rs. " + paid.Text;
